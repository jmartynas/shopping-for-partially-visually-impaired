...
func insertHandler(w http.ResponseWriter, r *http.Request) {
	...
	if count > 0 {
        ...
        WHERE pavadinimas = ?
    `
		err = db.QueryRow(checkQuery, parduotuvePavadinimas).Scan(&count)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}

		// Jei įrašas neegzistuoja, įterpkite naują
		if count == 0 {
			insertQuery := `
            INSERT INTO parduotuve (pavadinimas, nuotrauka)
            VALUES (?, ?)
        `
			_, err = db.Exec(insertQuery, parduotuvePavadinimas, parduotuveAdresas)
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
